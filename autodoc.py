...
class Autodoc(object):
    ...
    def generate(self, group="all", groups=[], sort=None):
        """Returns a list of dict of the routes specified by the doc() method
        ...
        for rule in current_app.url_map.iter_rules():
            ...
            if (groups and
                    [True for g in groups if func in self.groups[g]]) or \
                    (not groups and func in self.groups[group]):
                links.append(
                    dict(methods=rule.methods,
                         rule="%s" % rule,
    ...
    def html(self, template=None, group="all", groups=None, **context):
        ...
        A template can be specified. A list of routes is available under
        the 'autodoc' value (refer to the documentation for the generate() for
        a description of available values). If no template is specified,
        ...
        if template:
            ...
            filename = os.path.dirname(__file__) + \
                "/templates/autodoc_default.html"
